<template>
  ...
  const filteredAttendance = computed(() => {
    if (!search.value) {
      return attendanceRecords.value.map((item, index) => ({ ...item, number: index + 1 }));
    }
  ...
  const fetchAttendanceRecords = async () => {
    try {
      ...
      // Urutkan berdasarkan timestamp secara descending (terbaru ke terlama)
      attendanceRecords.value.sort((a, b) => {
        // Pastikan timestamp valid sebelum dibandingkan
        const timestampA = a.timestamp ? (a.timestamp.toDate ? a.timestamp.toDate().getTime() : new Date(a.timestamp).getTime()) : 0;
        const timestampB = b.timestamp ? (b.timestamp.toDate ? b.timestamp.toDate().getTime() : new Date(b.timestamp).getTime()) : 0;
